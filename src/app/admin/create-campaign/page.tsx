...
export default function CreateCampaignPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus({ type: 'error', message: err.message || 'Erro ao criar campanha.' });
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 sm:space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-12 sm:pb-20">
      <header className="flex items-start sm:items-center gap-4 sm:gap-6">
        <button 
          onClick={() => router.back()}
          className="h-11 w-11 sm:h-14 sm:w-14 rounded-2xl bg-white border border-slate-200 flex items-center justify-center hover:bg-slate-50 transition-all shadow-sm group shrink-0"
        >
          <ChevronLeft className="text-slate-600 group-hover:-translate-x-1 transition-transform" />
        </button>
        <div>
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-black text-slate-900 font-space italic uppercase leading-none">Nova <span className="text-blue-600">Campanha</span></h1>
          <p className="text-slate-500 text-sm sm:text-base font-medium tracking-tight mt-1">Configure os IDs de rastreio das plataformas Ads</p>
        </div>
      </header>
        ...
        <form onSubmit={handleSubmit} className="space-y-8 sm:space-y-12 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-10">
            ...
            {false && formData.platforms.includes('GOOGLE_ADS') && (
              <div className="space-y-6 p-5 sm:p-8 bg-yellow-50 border border-yellow-200 rounded-[2rem] sm:rounded-[2.5rem] animate-in zoom-in-95 duration-500">
            ...
            {formData.platforms.includes('META_ADS') && (
              <div className="space-y-6 p-5 sm:p-8 bg-blue-50 border border-blue-200 rounded-[2rem] sm:rounded-[2.5rem] animate-in zoom-in-95 duration-500">
                <div className="flex items-center gap-3 mb-2">
          ...
          <button 
            type="submit" 
            disabled={loading}
            className="w-full bg-slate-900 text-white font-black py-4 sm:py-6 rounded-[1.5rem] sm:rounded-[2rem] flex items-center justify-center gap-3 sm:gap-4 transition-all hover:bg-black active:scale-[0.98] shadow-2xl hover:shadow-slate-300 disabled:opacity-50 uppercase tracking-[0.2em] sm:tracking-[0.3em] text-[11px] sm:text-sm group"
          >
            {loading ? 'Processando...' : (
              <>
                Finalizar Cadastro
                <Send className="group-hover:translate-x-2 group-hover:-translate-y-1 transition-transform" size={20} />
              </>
            )}
          </button>
        </form>
...
function PlatformToggle({ active, onClick, icon, label, color }: any) {
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "flex items-center justify-between p-5 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] border-2 transition-all duration-300 group relative overflow-hidden",
        active 
          ? activeClasses 
    ...
    >
      <div className="flex items-center gap-3 sm:gap-5 relative z-10">
        <div className={cn(
          "h-10 w-10 sm:h-12 sm:w-12 rounded-2xl flex items-center justify-center transition-colors overflow-hidden p-2",
          active ? "bg-white text-slate-900" : "bg-slate-50 text-slate-400 group-hover:bg-slate-100"
        )}>
          {icon}
        </div>
        <span className="text-base sm:text-lg font-black italic uppercase tracking-tighter">{label}</span>
      </div>
      <div className={cn(
